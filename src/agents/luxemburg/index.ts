// AUTO-GENERATED scaffold for the "Rosa Luxemburg" agent.
...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`OPENAI_API_KEY_LUXEMBURG`] || process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "luxemburg". Set OPENAI_API_KEY_LUXEMBURG.`);
  ...
  if (!response.ok) {
    throw new Error(`Agent "luxemburg" API call failed: ${response.status} ${await response.text()}`);
  }
...
function parseModelOutput(data: any): DeliberationOutput {
  ...
  if (!toolCall) {
    throw new Error(`Agent "luxemburg" did not return a submit_deliberation tool call`);
  }
  let out: any = {};
  try {
    out = JSON.parse(toolCall.function.arguments);
  } catch {
    throw new Error(`Agent "luxemburg" returned malformed tool call arguments`);
  }
