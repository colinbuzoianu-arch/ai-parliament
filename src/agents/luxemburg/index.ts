...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`ANTHROPIC_API_KEY_LUXEMBURG`] ?? process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "luxemburg". Set ANTHROPIC_API_KEY_LUXEMBURG.`);
