// AUTO-GENERATED scaffold for the "Mohandas Gandhi" agent.
...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`OPENAI_API_KEY_GANDHI`] || process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "gandhi". Set OPENAI_API_KEY_GANDHI.`);
  ...
  if (!response.ok) {
    throw new Error(`Agent "gandhi" API call failed: ${response.status} ${await response.text()}`);
  }
...
function parseModelOutput(data: any): DeliberationOutput {
  ...
  if (!toolCall) {
    throw new Error(`Agent "gandhi" did not return a submit_deliberation tool call`);
  }
  let out: any = {};
  try {
    out = JSON.parse(toolCall.function.arguments);
  } catch {
    throw new Error(`Agent "gandhi" returned malformed tool call arguments`);
  }
