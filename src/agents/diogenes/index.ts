...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`ANTHROPIC_API_KEY_DIOGENES`] ?? process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "diogenes". Set ANTHROPIC_API_KEY_DIOGENES.`);
  ...
  if (!response.ok) {
    throw new Error(`Agent "diogenes" API call failed: ${response.status} ${await response.text()}`);
  }
