...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`OPENAI_API_KEY_IBNRUSHD`] || process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "ibnrushd". Set OPENAI_API_KEY_IBNRUSHD.`);
...
function parseModelOutput(data: any): DeliberationOutput {
  ...
  if (!toolCall) {
    throw new Error(`Agent "ibnrushd" did not return a submit_deliberation tool call`);
  }
  let out: any = {};
  try {
    out = JSON.parse(toolCall.function.arguments);
  } catch {
    throw new Error(`Agent "ibnrushd" returned malformed tool call arguments`);
  }
