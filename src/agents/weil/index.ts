// AUTO-GENERATED scaffold for the "Simone Weil" agent.
...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`OPENAI_API_KEY_WEIL`] || process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "weil". Set OPENAI_API_KEY_WEIL.`);
  ...
  if (!response.ok) {
    throw new Error(`Agent "weil" API call failed: ${response.status} ${await response.text()}`);
  }
...
function parseModelOutput(data: any): DeliberationOutput {
  ...
  if (!toolCall) {
    throw new Error(`Agent "weil" did not return a submit_deliberation tool call`);
  }
  let out: any = {};
  try {
    out = JSON.parse(toolCall.function.arguments);
  } catch {
    throw new Error(`Agent "weil" returned malformed tool call arguments`);
  }
