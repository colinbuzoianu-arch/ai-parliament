// AUTO-GENERATED scaffold for the "Etienne de La Boetie" agent.
...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`OPENAI_API_KEY_LABOETIE`] || process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "laboetie". Set OPENAI_API_KEY_LABOETIE.`);
  ...
  if (!response.ok) {
    throw new Error(`Agent "laboetie" API call failed: ${response.status} ${await response.text()}`);
  }
...
function parseModelOutput(data: any): DeliberationOutput {
  ...
  if (!toolCall) {
    throw new Error(`Agent "laboetie" did not return a submit_deliberation tool call`);
  }
  let out: any = {};
  try {
    out = JSON.parse(toolCall.function.arguments);
  } catch {
    throw new Error(`Agent "laboetie" returned malformed tool call arguments`);
  }
