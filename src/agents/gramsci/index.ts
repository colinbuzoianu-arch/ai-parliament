...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {
  const apiKey = process.env[`ANTHROPIC_API_KEY_GRAMSCI`] ?? process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "gramsci". Set ANTHROPIC_API_KEY_GRAMSCI.`);
  ...
  if (!response.ok) {
    throw new Error(`Agent "gramsci" API call failed: ${response.status} ${await response.text()}`);
  }
