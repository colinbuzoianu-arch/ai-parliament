...
export async function deliberate(input: DeliberationInput): Promise<DeliberationOutput> {{
  const apiKey = process.env[`ANTHROPIC_API_KEY_{ENV}`] ?? process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error(`Missing API key for agent "{id}". Set ANTHROPIC_API_KEY_{ENV}.`);
  ...
  if (!response.ok) {{
    throw new Error(`Agent "{id}" API call failed: ${{response.status}} ${{await response.text()}}`);
  }}
